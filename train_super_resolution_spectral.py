...
import sys
import logging
from models.super_resolution.Spectral_GNO import GraphFNO
from src.utilities import *
from src.dataset_super_res import Dataset
...
for ep in range(epochs):
    ...
    if ep%20 ==0:
        ...
        if dataset_name == "darcy":
            ...
            ax[0].set_title('Predicted Values', fontdict={'fontsize': 20, 'fontweight': 'bold', 'fontname': 'serif'})

            # Plot ground truth
            pic1 = ax[1].pcolormesh(X, Y, truth, cmap='RdBu_r', shading='gouraud', vmin=vmin, vmax=vmax)
            cbar1 = fig.colorbar(pic1, ax=ax[1])
            cbar1.ax.tick_params(labelsize=20)
            ax[1].set_title('Ground Truth', fontdict={'fontsize': 20, 'fontweight': 'bold', 'fontname': 'serif'})

            # Plot error
            pic2 = ax[2].pcolormesh(X, Y, error, cmap='RdBu_r', shading='gouraud')
            cbar2 = fig.colorbar(pic2, ax=ax[2])
            cbar2.ax.tick_params(labelsize=20)
            ax[2].set_title('Prediction Error', fontdict={'fontsize': 20, 'fontweight': 'bold', 'fontname': 'serif'})
